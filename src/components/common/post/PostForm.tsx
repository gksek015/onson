...
const PostForm = ({ categories, setFormData, formData }: PostFormProps) => {
  ...
  return (
    <div className="h-[calc(100vh-60px)] overflow-y-auto pb-20">
      <form onSubmit={handleSubmit}>
        <div className="mt-7 space-y-7 px-5">
          <div>
            <label htmlFor="title" className="tracking-[-0.5px] block text-base font-semibold">
              제목
            </label>
            <input
              type="text"
              ...
              className="tracking-[-0.4px] mt-3 block h-12 w-full rounded-[8px] border border-[#A1A6AA] px-3 text-base placeholder-[#868C92]"
          ...
          <div>
            <label htmlFor="tag" className="tracking-[-0.5px] block text-base font-semibold">
              태그
            </label>
          ...
          <div>
            <div className="flex items-center">
              <label htmlFor="content" className="tracking-[-0.5px] block text-base font-semibold">
                본문
              </label>
    ...
    ex) 봉사 시 강도가 높아 무거운 물건을 드는데 자신 있는 분을 찾습니다.`}
              rows={4}
              maxLength={500}
              className="mt-3 block w-full whitespace-pre-line rounded-[8px] border border-[#A1A6AA] px-2 py-2 text-base placeholder-[#868C92] shadow-sm leading-6"
              onChange={handleInputChange}
            />
          </div>
        </div>
