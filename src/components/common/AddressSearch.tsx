...
const AddressSearch = ({ onAddressSelect, option, onSelect }: AddressSearchProps) => {
  ...
  return (
    <div className="flex h-full flex-grow flex-col overflow-y-auto p-5 desktop:p-10">
      <h1 className="mb-6 text-2xl font-semibold leading-7 tracking-[-0.55px] desktop:text-4xl">위치</h1>
      ...
      <div className="relative flex w-auto items-center gap-2">
        <input
          ...
          className="my-4 w-full flex-1 cursor-pointer rounded-full border border-[#FB657E] p-0.5 px-5 py-3.5 text-base text-black shadow-input focus:outline-none desktop:mb-6 desktop:text-2xl"
        />
        <button
          type="submit"
          onClick={handleSearch}
          className="absolute right-5 top-1/2 -translate-y-1/2 text-[#4b4b4b] desktop:text-4xl"
        >
      ...
      <ul className="flex flex-col gap-2 overflow-y-scroll px-3 py-5 text-[#3C4043]">
        {searchResults.map((juso, index) => (
          <li
            key={index}
            className="tracking-content cursor-pointer py-2.5 text-base font-semibold desktop:text-2xl"
            onClick={() => {
